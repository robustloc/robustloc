...
class FeatDiffusion(nn.Module):
    ...
    def __init__(self, 
        ...
        n_heads,
        adj_h):
        super(FeatDiffusion, self).__init__()
    ...
    def forward(self, x):
        ...
        out = odeint(func=self.forward_attention, y0=x, t=self.integration_time, rtol=1e-3, atol=1e-3,
            method='euler')[-1]

        out = self.fcseq_norm(out)
        out = odeint(func=self.forward_fcseq, y0=out, t=self.integration_time, rtol=1e-3, atol=1e-3,
            method='euler')[-1]
