...
from network.vectdiffusion import VectDiffusion
from network.featdiffusion import FeatDiffusion
...
class RobustLoc(nn.Module):
    def __init__(self, feature_extractor, droprate=0.5, pretrained=True, 
        ...
        self.gatt = VectDiffusion(512, out_features=None, hidden_features=64, n_layers=3, n_heads=8)
        ...
        self.vvit_l40 = FeatDiffusion(in_features=512, hidden_features=64, n_heads=8, adj_h=opt.subseq_length*(4**2))
...
if __name__ == '__main__':
    import resnet
    model = RobustLoc(feature_extractor=resnet.resnet34(pretrained=True))

    input = torch.rand([1,9,3,128,128])
    output = model(input)
